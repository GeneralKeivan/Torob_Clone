...
var first;
var found = false;
class CustomerTabletApple extends Component {
    ...
    handleUpdate(event) {
        ...
        for(var i = 0; i < products.length; i++){
            if(String(products[i].name.toLowerCase().includes(searchBar)) && products[i].brand === "apple" && products[i].model === "tablet"){
    ...
    render() {
      ...
      if(first){
        ...
        for(var i = 0; i < products.length; i++){
            if(products[i].model === "tablet" && products[i].brand === "apple"){
...
export default connect(mapStateToProps, mapDispatchToProps)(CustomerTabletApple);